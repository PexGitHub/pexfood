...
class _AddFoodMenuState extends State<AddFoodMenu> {
          ...
          onPressed: () {
            if (file == null) {
              normalDialog(context,
                  'กรุณาเลือกรูปภาพ อาหาร โดยการเลือกจาก Camera หรือ Gallery');
  ...
  Future<Null> uploadFoodAndInsertData() async {
    String urlUpload = '${MyConstant().domain}/pexfood/saveFood.php';
    ...
    try {
      ...
      await Dio().post(urlUpload, data: formData).then((value) async {
        String urlPathImage = '/pexfood/food/$namefile';
        print('Url = ${MyConstant().domain}$urlPathImage');

        SharedPreferences preferences = await SharedPreferences.getInstance();
        String idShop = preferences.getString('id');

        String urlInsertData =
            '${MyConstant().domain}/pexfood/addFood.php?isAdd=true&idShop=$idShop&nameFood=$nameFood&pathImage=$urlPathImage&price=$price&detail=$detail';
